...
<template>
      ...
      <div class="dashboard-card fade-in" :class="{ visible: fadeInVisible }">
        <div class="filtros">
          <button
            ...
            :key="s.valor"
            class="btn-filtro"
            :class="{ ativo: filtroStatus === s.valor }"
            @click="filtroStatus = s.valor"
          >
            {{ s.label }}
          </button>
        </div>
            ...
            <tbody>
              <tr v-for="a in assinaturasFiltradas" :key="a.id">
                ...
                <td>
                  ...
                  <span v-else class="sem-acao">—</span>
                </td>
              </tr>
  ...
  async function cancelar(id) {
    ...
    try {
      await api.patch(`/assinaturas/admin/${id}/cancelar`)
      const a = assinaturas.value.find(x => x.id === id)
  ...
  .btn-cancelar:hover { background: rgba(244,67,54,.3); transform:translateY(-2px); }
